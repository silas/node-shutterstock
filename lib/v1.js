  ...
  req.end(function(err, res) {
    ...
    if (res.statusCode >= 400) {
      ...
      if (contentType && ~contentType.indexOf('text/plain')) {
        return cb(error(res.text, res));
      }

      return cb(error(http.STATUS_CODES[res.statusCode], res));
    }
...
Shutterstock.prototype.customerImageDownloads = function(options, cb) {
  ...
  return this.wrap(
    ...
    _.pick(options, 'auth_token', 'user'),
    cb
  );
};
...
Shutterstock.prototype.customerSubscriptions = function(options, cb) {
  if (typeof options === 'function') {
    cb = options;
    options = {};
  }
  ...
  return this.wrap(
    request.get(this.url('customers', options.auth_user, 'subscriptions')),
    _.pick(options, 'auth_token', 'user'),
    cb
  );
};
